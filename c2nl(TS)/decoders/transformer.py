...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.context_attn2 = MultiHeadedAttention(
            heads, d_model, d_k, d_v, dropout=dropout,
            coverage=coverage_attn)
        self.layer_norm_3 = LayerNorm(d_model)
        self.layer_norm_4 = LayerNorm(d_model)

        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.drop = nn.Dropout(dropout)
        self.weight = torch.nn.Parameter(torch.tensor([1.0]))
        self.weight2 = torch.nn.Parameter(torch.tensor([0.0]))
    ...
    def forward(self,
        ...
        mid2, attn2, coverage2 = self.context_attn2(memory_bank2,
                                                    memory_bank2,
                                                    query_norm,
                                                    mask=src2_pad_mask,
                                                    attn_type="context",
                                                    coverage=coverage)
        mid_norm2 = self.layer_norm_3(self.drop(mid2) + query_norm)

        weighted_mid = self.weight * mid_norm
        weighted_mid2 = self.weight2 * mid_norm2
        final_mid = self.layer_norm_4(weighted_mid + weighted_mid2)

        output = self.feed_forward(final_mid)

        return output, attn2, coverage2
...
class TransformerDecoder(DecoderBase):
    ...
    def init_state(self, src_len, max_len,src2_len,max_len2):
        """Initialize decoder state."""
        state = dict()
        state["src_len"] = src_len  # [B]
        state["src_max_len"] = max_len  # an integer
        state["src2_len"] = src2_len
        state["src2_max_len"] = max_len2
        state["cache"] = None
        return state
    ...
    def forward(self,
        ...
        src_pad_mask2 = ~sequence_mask(state["src2_len"],
                                      max_len=state["src2_max_len"]).unsqueeze(1)
        ...
        for i, layer in enumerate(self.layer):
            ...
            mem_bank2 = memory_bank2[i] if isinstance(memory_bank2, list) else memory_bank2
