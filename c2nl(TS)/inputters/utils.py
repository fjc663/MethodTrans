...
def process_examples(lang_id,
    ...
    code2_tokens = source2.split()
    code2_type = []

    code2_tokens = code2_tokens[:max_src2_len]
    code2_type = code2_type[:max_src2_len]
    if len(code2_tokens) == 0:
        return None

    code2 = Code()
    code2.text = source2
    code2.language = lang_id
    code2.tokens = code2_tokens
    code2.type = [TAG_TYPE_MAP.get(ct, 1) for ct in code2_type]
...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    with open(filenames['src_2']) as f:
        sources_2 = [line.strip() for line in
                   tqdm(f, total=count_file_lines(filenames['src_2']))]
    ...
    assert len(sources) ==len(source_tags) == len(source_tags) == len(targets)

    examples = []
    for src, src_2, src_tag, tgt in tqdm(zip(sources, sources_2, source_tags, targets),
                                  total=len(sources)):
