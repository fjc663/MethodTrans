...
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        #assert args.use_src2_word or args.use_src2_char
        assert args.use_tgt_word or args.use_tgt_char

        self.use_src_word = args.use_src_word
        #self.use_src2_word = args.use_src2_word
        ...
        if self.use_src_word:
            self.src_word_embeddings = Embeddings(args.emsize,
                                                  args.src_vocab_size,
                                                  constants.PAD)
            self.src2_word_embeddings = Embeddings(args.emsize,
                                                  args.src2_vocab_size,
                                                  constants.PAD)
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            ...
            word_rep = None
            if self.use_src_word:
                word_rep = self.src2_word_embeddings(sequence.unsqueeze(2))  # B x P x d
...
class Encoder(nn.Module):
    ...
    def forward(self,
                ...
                input2=None,
                input_len2=None):
        layer_outputs, _ = self.transformer(input, input_len,input2,input_len2)  # B x seq_len x h
        if self.use_all_enc_layers:
...
class Decoder(nn.Module):
    ...
    def init_decoder(self,
        ...
        if self.split_decoder:
            state_c = self.transformer_c.init_state(src_lens, max_src_len,src2_lens,max_src2_len)
            state_d = self.transformer_d.init_state(src_lens, max_src_len,src2_lens,max_src2_len)
            return state_c, state_d
        else:
            return self.transformer.init_state(src_lens, max_src_len,src2_lens,max_src2_len)
    ...
    def forward(self,
        ...
        max_mem_len2 = memory_bank2[0].shape[1] \
            if isinstance(memory_bank2, list) else memory_bank2.shape[1]
        state = self.init_decoder(memory_len, max_mem_len,memory_len2,max_mem_len2)
        return self.decode(tgt_pad_mask, tgt_emb, memory_bank,memory_bank2, state)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        self.encoder2 = Encoder(args, self.embedder.enc_input_size)
        #self.fusion=nn.Linear(self.embedder.dec_input_size*2,self.embedder.dec_input_size)
    ...
    def _run_forward_ml(self,
        ...
        # embed and encode the source2 sequence
        code2_rep = self.embedder(code2_word_rep,
                                  code2_char_rep,
                                  code2_type_rep,
                                  mode='encoder2')

        #TODO :关注memory bank
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len,code2_rep, code2_len)  # B x seq_len x h

        memory_bank2, layer_wise_outputs2 = self.encoder2(code2_rep, code2_len,code_rep, code_len)  # B x seq_len x h
        ...
        summ_emb = self.embedder(summ_word_rep,
                                 summ_char_rep,
                                 mode='decoder')
        #波浪号运算符，补码：https://www.nhooo.com/note/qa09jj.html
        ...
        enc2_outputs = layer_wise_outputs2 if self.layer_wise_attn else memory_bank2

        # concat
        #enc_outputs=torch.cat((enc_outputs,enc2_outputs),-1)
    ...
    def __generate_sequence(self,
        ...
        dec_states = self.decoder.init_decoder(params['src_len'], max_mem_len,params['src2_len'],max_mem_len2)

        attns = {"coverage": None}
        enc_outputs = params['layer_wise_outputs'] if self.layer_wise_attn \
            else params['memory_bank']

        enc2_outputs = params['layer_wise_outputs2'] if self.layer_wise_attn \
            else params['memory_bank2']
    ...
    def decode(self,
        ...
        word_rep = self.embedder(code_word_rep,
                                 ...
                                 code_type_rep,
                                 mode='encoder')
        word_rep2 = self.embedder(code2_word_rep,
                                 code2_char_rep,
                                 code2_type_rep,
                                 mode='encoder2')

        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len,word_rep2, code2_len)  # B x seq_len x h
        memory_bank2, layer_wise_outputs2 = self.encoder2(word_rep2, code2_len,word_rep, code_len)
    ...
    def count_encoder_parameters(self):
        return self.encoder.count_parameters()+self.encoder2.count_parameters()
