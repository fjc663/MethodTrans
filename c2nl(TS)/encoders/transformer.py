...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attention2 = MultiHeadedAttention(heads,
                                              d_model,
                                              d_k,
                                              d_v,
                                              dropout=dropout,
                                              max_relative_positions=max_relative_positions,
                                              use_neg_dist=use_neg_dist)
        self.attention3 = MultiHeadedAttention(heads,
                                              d_model,
                                              d_k,
                                              d_v,
                                              dropout=dropout,
    ...
    def forward(self, inputs, mask,inputs2=None,mask2=None,l=None):
        """
        ...
        context = self.layer_norm(self.dropout(context) + inputs)


        context2, attn_per_head2, _ = self.attention2(inputs2, inputs2, inputs2,
                                                   mask=mask2, attn_type="self")
        context2 = self.layer_norm2(self.dropout(context2) + inputs2)

        context3, attn_per_head3, _ = self.attention3(context2, context2, context,
                                                   mask=mask2, attn_type="context")
        out = self.layer_norm3(self.dropout(context3) + context)

        return self.feed_forward(out), attn_per_head3
...
class TransformerEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None,src2=None,lengths2=None):
        """
        Args:
        ...
        self._check_args(src2, lengths2)

        out = src
        out2 = src2
        mask = None if lengths is None else \
            ~sequence_mask(lengths, out.shape[1]).unsqueeze(1)
        mask2 = None if lengths2 is None else \
            ~sequence_mask(lengths2, out2.shape[1]).unsqueeze(1)
        # Run the forward pass of every layer of the tranformer.
        representations = []
        attention_scores = []
        for i in range(self.num_layers):
            out, attn_per_head = self.layer[i](out, mask,out2,mask2)
