...
def add_train_args(parser):
    ...
    files.add_argument('--train_src2', nargs='+', type=str,
                       help='Preprocessed train source file 2')
    ...
    files.add_argument('--dev_src2', nargs='+', type=str, required=True,
                       help='Preprocessed dev source file 2')
    ...
    preprocess.add_argument('--src2_vocab_size', type=int, default=None,
                            help='Maximum allowed length for src2 dictionary')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        for i in range(num_dataset):
            ...
            train_src2 = os.path.join(data_dir, args.train_src2[i])
            ...
            args.train_src2_files.append(train_src2)
    ...
    for i in range(num_dataset):
        ...
        dev_src2 = os.path.join(data_dir, args.dev_src2[i])
        ...
        args.dev_src2_files.append(dev_src2)
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    src2_dict = util.build_word_and_char_dict(args,
                                             examples=train_exs + dev_exs,
                                             fields=['code2'],
                                             dict_size=args.src2_vocab_size,
                                             no_special_token=True)
    ...
    # Initialize model
    model = Code2NaturalLanguage(config.get_model_args(args), src_dict,src2_dict, tgt_dict)
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src2, train_src_tag, train_tgt, dataset_name in \
                zip(args.train_src_files, args.train_src2_files, args.train_src_tag_files,
                    args.train_tgt_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_2'] = train_src2
    ...
    dev_exs = []
    for dev_src,dev_src2, dev_src_tag, dev_tgt, dataset_name in \
            zip(args.dev_src_files, args.dev_src2_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dataset_name):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_2'] = dev_src2
