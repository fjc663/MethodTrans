...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, args, src_dict,src2_dict, tgt_dict, state_dict=None):
        # Book-keeping.
        self.args = args
        self.src_dict = src_dict
        self.args.src_vocab_size = len(src_dict)
        self.src2_dict = src2_dict
        self.args.src2_vocab_size = len(src2_dict)
    ...
    def init_optimizer(self, state_dict=None, use_gpu=True):
        ...
        """
        #TODO: what is fix embeddign?
        if self.args.fix_embeddings:
            self.network.embedder.src_word_embeddings.fix_word_lut()
            self.network.embedder.src2_word_embeddings.fix_word_lut()
    ...
    def update(self, ex):
        ...
        source_map,source2_map, alignment = None,None, None
        ...
        if self.args.copy_attn:
            assert 'src_map' in ex and 'src2_map' in ex and 'alignment' in ex

            source_map = make_src_map(ex['src_map'])
            source_map = source_map.cuda(non_blocking=True) if self.use_cuda \
                else source_map

            source2_map = make_src_map(ex['src2_map'])
            source2_map = source2_map.cuda(non_blocking=True) if self.use_cuda \
                else source2_map
        ...
        code2_word_rep = ex['code2_word_rep']
        code2_char_rep = ex['code2_char_rep']
        code2_type_rep = ex['code2_type_rep']
        code2_mask_rep = ex['code2_mask_rep']
        code2_len = ex['code2_len']
        ...
        if self.use_cuda:
            ...
            code2_len = code2_len.cuda(non_blocking=True)
            ...
            if code2_word_rep is not None:
                code2_word_rep = code2_word_rep.cuda(non_blocking=True)
            if code2_char_rep is not None:
                code2_char_rep = code2_char_rep.cuda(non_blocking=True)
            if code2_type_rep is not None:
                code_type_rep = code2_type_rep.cuda(non_blocking=True)
            if code2_mask_rep is not None:
                code2_mask_rep = code2_mask_rep.cuda(non_blocking=True)
        ...
        net_loss = self.network(code_word_rep=code_word_rep,
                                ...
                                tgt_seq=tgt_seq,
                                src_map=source_map,
                                src2_map=source2_map,
                                alignment=alignment,
                                src_dict=self.src_dict,
                                src2_dict=self.src2_dict,
                                tgt_dict=self.tgt_dict,
                                max_len=self.args.max_tgt_len,
                                blank=blank,
                                fill=fill,
                                source_vocab=ex['src_vocab'],
                                source2_vocab=ex['src2_vocab'],
    ...
    def predict(self, ex, replace_unk=False):
        ...
        if self.use_cuda:
            ...
            if code2_char_rep is not None:
                code2_char_rep = code2_char_rep.cuda(non_blocking=True)
            if code2_type_rep is not None:
                code2_type_rep = code2_type_rep.cuda(non_blocking=True)
            if code2_mask_rep is not None:
                code2_mask_rep = code2_mask_rep.cuda(non_blocking=True)
    ...
    def load(filename, new_args=None):
        ...
        src2_dict = saved_params['src2_dict']
        tgt_dict = saved_params['tgt_dict']
        state_dict = saved_params['state_dict']
        args = saved_params['args']
        if new_args:
            args = override_model_args(args, new_args)
        return Code2NaturalLanguage(args, src_dict, src2_dict, tgt_dict, state_dict)
    ...
    def load_checkpoint(filename, use_gpu=True):
        ...
        args = saved_params['args']
        model = Code2NaturalLanguage(args, src_dict, src2_dict, tgt_dict, state_dict)
