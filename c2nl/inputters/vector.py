...
def vectorize(ex, model):
    ...
    src2_dict = model.src2_dict
    tgt_dict = model.tgt_dict

    code,code2, summary = ex['code'],ex['code2'], ex['summary']
    vectorized_ex = dict()
    ...
    vectorized_ex['code2'] = code2.text
    vectorized_ex['code2_tokens'] = code2.tokens
    vectorized_ex['code2_char_rep'] = None
    vectorized_ex['code2_type_rep'] = None
    vectorized_ex['code2_mask_rep'] = None
    vectorized_ex['use_code2_mask'] = False



    vectorized_ex['code_word_rep'] = torch.LongTensor(code.vectorize(word_dict=src_dict))
    vectorized_ex['code2_word_rep'] = torch.LongTensor(code2.vectorize(word_dict=src2_dict))


    #TODO： 考虑到不适用char_rep 不改
    ...
    vectorized_ex['src2_vocab'] = code2.src_vocab
...
def batchify(batch):
    ...
    code2_words = [ex['code2_word_rep'] for ex in batch]
    code2_chars = [ex['code2_char_rep'] for ex in batch]
    code2_type = [ex['code2_type_rep'] for ex in batch]
    code2_mask = [ex['code2_mask_rep'] for ex in batch]
    max_code2_len = max([d.size(0) for d in code2_words])
    ...
    if use_src_char:
        ...
        max_char_in_code2_token = code_chars[0].size(1)
    ...
    code2_len_rep = torch.zeros(batch_size, dtype=torch.long)
    code2_word_rep = torch.zeros(batch_size, max_code2_len, dtype=torch.long) \
        if use_src_word else None
    code2_type_rep = torch.zeros(batch_size, max_code2_len, dtype=torch.long) \
        if use_code_type else None
    code2_mask_rep = torch.zeros(batch_size, max_code2_len, dtype=torch.long) \
        if use_code_mask else None
    code2_char_rep = torch.zeros(batch_size, max_code2_len, max_char_in_code2_token, dtype=torch.long) \
        if use_src_char else None
    ...
    src2_vocabs = []
    for i in range(batch_size):
        code2_len_rep[i] = code2_words[i].size(0)
        if use_src_word:
            code2_word_rep[i, :code2_words[i].size(0)].copy_(code2_words[i])
        if use_code_type:
            code2_type_rep[i, :code2_type[i].size(0)].copy_(code2_type[i])
        if use_code_mask:
            code2_mask_rep[i, :code2_mask[i].size(0)].copy_(code2_mask[i])
        if use_src_char:
            code2_char_rep[i, :code2_chars[i].size(0), :].copy_(code2_chars[i])
        #
        context2 = batch[i]['code2_tokens']
        vocab2 = batch[i]['src2_vocab']
        src2_vocabs.append(vocab2)
        # Mapping source tokens to indices in the dynamic dict.
        src2_map = torch.LongTensor([vocab2[w] for w in context2])
        source2_maps.append(src2_map)
    ...
    return {
        ...
        'code2_word_rep': code2_word_rep,
        'code2_char_rep': code2_char_rep,
        'code2_type_rep': code2_type_rep,
        'code2_mask_rep': code2_mask_rep,
        'code2_len': code2_len_rep,
        ...
        'src_map': source_maps,
        'src2_vocab': src2_vocabs,
        'src2_map': source2_maps,
